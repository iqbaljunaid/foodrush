...
export async function notificationRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post<{ Body: SendNotificationInput }>('/notifications', async (request, reply) => {
    const { recipientId, channel, destination, subject, body, metadata } = request.body;

    if (!recipientId || !channel || !destination || !body) {
      return reply.status(400).send({ error: 'recipientId, channel, destination, and body are required' });
    }
