...
export async function paymentRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post<{ Body: CreatePaymentInput }>('/payments', async (request, reply) => {
    const { orderId, customerId, amount, currency, paymentMethodId, idempotencyKey } = request.body;

    if (!orderId || !customerId || !amount || !paymentMethodId || !idempotencyKey) {
      return reply.status(400).send({ error: 'orderId, customerId, amount, paymentMethodId, and idempotencyKey are required' });
    }
    ...
    try {
      ...
      app.log.error(`Stripe payment failed: ${err instanceof Error ? err.message : String(err)}`);
    }

    await pool.execute<ResultSetHeader>(
      `INSERT INTO payments (id, order_id, customer_id, amount, currency, status, stripe_payment_intent_id, idempotency_key)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [id, orderId, customerId, amount, paymentCurrency, status, stripePaymentIntentId, idempotencyKey],
    );
    ...
    const row = rows[0];
    if (!row) {
      return reply.status(500).send({ error: 'Failed to retrieve created payment' });
    }
  ...
  app.get<{ Params: { orderId: string } }>('/payments/order/:orderId', async (request, reply) => {
    const pool = getPool();
    const [rows] = await pool.execute<(PaymentRow & RowDataPacket)[]>(
    ...
    async (request, reply) => {
      ...
      try {
        ...
        if (row.status !== PaymentStatus.AUTHORIZED) {
          await connection.rollback();
          return reply.status(409).send({ error: `Cannot capture payment in status: ${row.status}` });
        }

        if (!row.stripe_payment_intent_id) {
          await connection.rollback();
          return reply.status(409).send({ error: 'No Stripe payment intent associated' });
        }

        const amountToCapture = request.body.amountToCapture;
        await capturePaymentIntent(row.stripe_payment_intent_id, amountToCapture);
        ...
        if (!updatedRow) {
          return reply.status(500).send({ error: 'Failed to retrieve updated payment' });
        }
  ...
  app.post<{ Params: { id: string }; Body: RefundPaymentInput }>(
    ...
    async (request, reply) => {
      ...
      try {
        ...
        if (row.status !== PaymentStatus.CAPTURED && row.status !== PaymentStatus.PARTIALLY_REFUNDED) {
          await connection.rollback();
          return reply.status(409).send({ error: `Cannot refund payment in status: ${row.status}` });
        }
        ...
        const refundAmount = request.body.amount ?? Number(row.amount) - Number(row.refunded_amount);
        const maxRefundable = Number(row.amount) - Number(row.refunded_amount);

        if (refundAmount <= 0 || refundAmount > maxRefundable) {
          await connection.rollback();
          return reply.status(400).send({ error: `Invalid refund amount. Max refundable: ${maxRefundable}` });
        }

        await createRefund(row.stripe_payment_intent_id, refundAmount, request.body.reason);
