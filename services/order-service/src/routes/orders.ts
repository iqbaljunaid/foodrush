...
import { validateTransition, InvalidTransitionError } from '../fsm/order-fsm.js';
import { publishOrderEvent } from '../events/producer.js';
import { TOPICS, buildOrderPlacedEvent, buildOrderStatusChangedEvent } from '../events/schemas.js';
import { OrderStatus } from '../types/index.js';
import type { CreateOrderInput, OrderRow, OrderStatusType } from '../types/index.js';
import type { ResultSetHeader, RowDataPacket } from 'mysql2';

export async function orderRoutes(app: FastifyInstance): Promise<void> {
  // Create order
  app.post<{ Body: CreateOrderInput }>('/orders', async (request, reply) => {
    const { customerId, restaurantId, items, deliveryAddress, notes } = request.body;
    ...
    await pool.execute<ResultSetHeader>(
      `INSERT INTO orders (id, customer_id, restaurant_id, items, status, total_amount, delivery_address, notes)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [id, customerId, restaurantId, JSON.stringify(items), OrderStatus.PLACED, totalAmount, deliveryAddress, notes ?? null],
    );
  ...
  app.get<{ Querystring: { customerId: string; limit?: string; offset?: string } }>(
    '/orders',
    async (request, reply) => {
  ...
  app.patch<{
    ...
    if (!toStatus || !actorId) {
      return reply.status(400).send({ error: 'status and actorId are required' });
    }
    ...
    try {
      ...
      if (!updatedRow) {
        return reply.status(500).send({ error: 'Failed to retrieve updated order' });
      }
      ...
      await publishOrderEvent(
        ...
        buildOrderStatusChangedEvent(updatedOrder, currentStatus, actorId, reason),
      );
