 ...
 */
const TRANSITIONS: ReadonlyMap<OrderStatusType, ReadonlySet<OrderStatusType>> = new Map([
  [OrderStatus.PLACED, new Set([OrderStatus.ACCEPTED, OrderStatus.CANCELLED])],
  [OrderStatus.ACCEPTED, new Set([OrderStatus.PREPARING, OrderStatus.CANCELLED])],
...
export function getValidTransitions(from: OrderStatusType): readonly OrderStatusType[] {
  const allowed = TRANSITIONS.get(from);
  if (!allowed) {
    return [];
  }
