...
import type { CreateCatalogueInput, UpdateCatalogueInput, Catalogue } from '../types/index.js';
...
export async function catalogueRoutes(app: FastifyInstance): Promise<void> {
  ...
  // Create catalogue
  app.post<{ Body: CreateCatalogueInput }>('/catalogues', async (request, reply) => {
    const { restaurantId, name, description, category } = request.body;

    if (!restaurantId || !name || !description || !category) {
      return reply.status(400).send({ error: 'Missing required fields: restaurantId, name, description, category' });
    }
  ...
  // Get catalogue by ID
  app.get<{ Params: { id: string } }>('/catalogues/:id', async (request, reply) => {
    const client = getNoSQLClient();
  ...
  app.get<{ Querystring: { restaurantId: string; limit?: string; offset?: string } }>(
    '/catalogues',
    async (request, reply) => {
      ...
      const catalogues = result.rows.map((row) =>
        rowToCatalogue(row as Record<string, unknown>),
      );
  ...
  // Delete catalogue
  app.delete<{ Params: { id: string } }>('/catalogues/:id', async (request, reply) => {
    const { id } = request.params;
    const client = getNoSQLClient();
