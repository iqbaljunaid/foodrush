...
import { updateLocation, findNearby, getLocation, removeLocation } from '../geo/redis-geo.js';
import { checkGeofences, registerGeofence, removeGeofence, listGeofences } from '../geo/geofence.js';
...
export async function locationRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.put<{ Body: LocationUpdate }>('/locations', async (request, reply) => {
    const { entityId, entityType, latitude, longitude, timestamp } = request.body;
  ...
  // Get entity location
  app.get<{ Params: { entityId: string } }>('/locations/:entityId', async (request, reply) => {
  ...
  app.get<{
    ...
    const { entityType, latitude, longitude, radiusKm = '5', limit = '10' } = request.query;

    if (!entityType || !latitude || !longitude) {
      return reply.status(400).send({ error: 'entityType, latitude, and longitude are required' });
    }
  ...
  app.delete<{ Params: { entityId: string }; Querystring: { entityType: string } }>(
    '/locations/:entityId',
    async (request, reply) => {
      const { entityType } = request.query;

      if (!entityType) {
        return reply.status(400).send({ error: 'entityType query parameter is required' });
      }
  ...
  // Delete geofence
  app.delete<{ Params: { id: string } }>('/geofences/:id', async (request, reply) => {
