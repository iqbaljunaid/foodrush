...
import { orderApi, type OrderItem, dispatchApi } from '@foodrush/shared/api';
...
// ── Types ───────────────────────────────────────────────────────────
type RouteProp = NativeStackScreenProps<DeliveriesStackParamList, 'Pickup'>['route'];
type NavProp = NativeStackNavigationProp<DeliveriesStackParamList, 'Pickup'>;

interface ChecklistItem extends OrderItem {
  checked: boolean;
}
...
export function PickupScreen(): React.JSX.Element {
  ...
  const handleTakePhoto = useCallback(async () => {
    if (!cameraPermission?.granted) {
      ...
      if (!result.granted) {
        Alert.alert(
          'Camera Permission',
          'Camera access is needed to take a pickup photo.',
        );
        return;
      }
    }
  ...
  const handleConfirmPickup = useCallback(async () => {
    if (!allChecked) {
      Alert.alert('Incomplete', 'Please check all items before confirming.');
      return;
    }
  ...
  }, [allChecked, deliveryId, navigation]);
  ...
  return (
    <View style={styles.screen}>
      ...
      >
        ...
        {/* Photo section */}
        <Text style={styles.sectionTitle}>Pickup Photo</Text>
        {photoUri ? (
          <View style={styles.photoPreview}>
            <Image
              source={{ uri: photoUri }}
              style={styles.photoImage}
              accessibilityLabel="Pickup photo preview"
            />
            <Pressable
